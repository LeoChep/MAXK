...
public class SolutionBuilder {
	...
	public void reconsitByRow(int rowLable) {
		System.out.println(rowLable);
		for (int i = 0; i < rows[rowLable].getBeCoverBy().size(); i++) {
			Column column = rows[rowLable].getBeCoverBy().get(i);
			System.out.println(column.getLebel());
			if (column.getEvaluetion() == -1) {
				System.out.println("undo@!!!!!!!!!!!!!!!!!!!!!!" + rowLable);
				return;
			}
			// if (column.getEvaluetion()==0) evaluet(column);
			if (column.isPick()) {
				getSolution().getByEffect()[column.getEvaluetion()].getColumns().remove(column);
				int evaluetion = evaluet(column);
				getSolution().getByEffect()[evaluetion].getColumns().add(column);
			} else {
				antiSolution.getByEffect()[column.getEvaluetion()].getColumns().remove(column);
				int evaluetion = antiEvaluet(column);
				antiSolution.getByEffect()[evaluetion].getColumns().add(column);
			}
		}
	}
...
//		System.out.println(getSolution().getByEffect()[gain_a].getColumns().size());
		...
		outColumn.setEvaluetion(-1);
		outColumn.setPick(false);
		for (int i = 0; i < outColumn.getIsConverRow().size(); i++) {// remove时要修改解的覆盖情况
			int dateRowLabel = outColumn.getIsConverRow().get(i).getLebel();
			System.out.println("the row is:" + dateRowLabel);
			System.out.println("the COLUMN is:" + outColumn.getLebel());
			getSolution().getDeta()[dateRowLabel]--;// 需要重写remove方法，此为测试
		}
		for (int i = 0; i < outColumn.getIsConverRow().size(); i++) {// remove时要修改解的覆盖情况
			int dateRowLabel = outColumn.getIsConverRow().get(i).getLebel();
			if (getSolution().getDeta()[dateRowLabel] == 1 || getSolution().getDeta()[dateRowLabel] == 0 || true) {

				System.out.println("the COLUMN size is:" + outColumn.getIsConverRow().size());
				this.reconsitByRow(dateRowLabel);
			}
		}
		// this.reconsit();//重新调整解结构
		outColumn.setEvaluetion(0);
		int gain_b = antiSolution.getMaxUsebleEffect(sa.allCount);// 计算b的影响值
		System.out.println(gain_b);
		if (gain_b == -1) {// 寻找不到可移动列
			getInstruct(0);
			this.getSolution().getByEffect()[0].getColumns().add(outColumn);
			outColumn.setEvaluetion(0);
			outColumn.setPick(true);
			for (int i = 0; i < outColumn.getIsConverRow().size(); i++) {// remove时要修改解的覆盖情况
				int dateRowLabel = outColumn.getIsConverRow().get(i).getLebel();
				getSolution().getDeta()[dateRowLabel]++;// 需要重写remove方法，此为测试
			}
		...
		if (getInstruct(gain_b - gain_a)) {// 如果符合条件，则进行交换，否则复原
			this.antiSolution.getByEffect()[0].getColumns().add(outColumn);// 将A加入非解
			outColumn.setEvaluetion(0);
			...
			}
			{
				...
				inColumn.setPick(true);
				// inColumn.setEvaluetion(evaluet(inColumn));
				inColumn.setEvaluetion(0);
				this.antiSolution.getByEffect()[gain_b].getColumns().remove(random);
				getSolution().getByEffect()[0].getColumns().add(inColumn);

				for (int i = 0; i < inColumn.getIsConverRow().size(); i++) {
					getSolution().getDeta()[inColumn.getIsConverRow().get(i).getLebel()]++;// 需要重写add方法，此为测试
				}
				for (int i = 0; i < inColumn.getIsConverRow().size(); i++) {
					if (getSolution().getDeta()[inColumn.getIsConverRow().get(i).getLebel()] == 1
							|| getSolution().getDeta()[inColumn.getIsConverRow().get(i).getLebel()] == 0 || true) {
						this.reconsitByRow(inColumn.getIsConverRow().get(i).getLebel());
					}
				}
			}
			// 随机抽取B，将B出列并加入解中
